...
import com.viaplay.menu.TopMenu;
import com.viaplay.page.RootPage;
import com.viaplay.selector.Via;
import com.viaplay.selector.ViaCss;
import com.viaplay.selector.ViaId;

/**
 * This class wraps and simplifies the selection and construction of a Selenium Web Browser.
 * 
...
public class Browser {
	...
	public Browser get(String urlAsString) {
		...
		waitForElement(Via.id(TopMenu.TOPMENU_LOGO));
		return this;
	}
	...
	/**
	 * Convenient helper method that log a user in to the web page.
	 * 
	 * @param user
	 *            The UserData object of an user.
	 */
	public void login(UserData user) {
		log.info("Logging in user {}.", user.getUsername());
		clickElement(Via.id(TopMenu.TOPMENU_LOGIN));
		sendKeysToElement(user.getUsername(), Via.id(TopMenu.TOPMENU_LOGIN_INPUT_LOGIN_NAME));
		sendKeysToElement(user.getPassword(), Via.id(TopMenu.TOPMENU_LOGIN_INPUT_LOGIN_PASS));
		clickElement(Via.id(TopMenu.TOPMENU_LOGIN_SUBMIT));
		waitForElementToDisappear(Via.id(TopMenu.TOPMENU_LOGIN_DROPDOWN));
		waitForElement(Via.id(TopMenu.TOPMENU_LOGO));
	}

	/**
	 * Convenient helper method that do log out the user from the web page.
	 */
	public void logout() {
		get(RootPage.URL);
		if (isVisible(Via.id(TopMenu.TOPMENU_MY_VIAPLAY))) {
			log.info("Logging out user.");
			clickElement(Via.id(TopMenu.TOPMENU_MY_VIAPLAY));
			clickElement(Via.id(TopMenu.TOPMENU_LOGOUT_LINK));
			waitForElementToDisappear(Via.id(TopMenu.TOPMENU_LOGIN_DROPDOWN));
			waitForCookieToDisappear("OPS");
		} else {
			log.info("The user seems to be logged out already - Skipping!");
		}
	}
